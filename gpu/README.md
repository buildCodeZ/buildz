...
原理：
    ...
    如果假设模型里每层网络的计算开销是一样的，缓存最大上限设置成实际需要缓存大小的50%，会增加16.7%的计算开销；设置为25%，会增加50%的计算开销；设置为12.5%，会增加116.7%的计算开销，也就是训练时间变成两倍多。实际不同网络层的计算开销不一样，需要使用者自己测试
    注1：pytorch有个torch.utils.checkpoint，是类似的作用，不过它可以设置哪些网络层缓存不丢失，避免计算开销大的网络层重新计算，但使用起来比较复杂，只是在网上看过，没具体使用过
    注2：函数名里的rngs表示会调用torch.cuda.get_rng_state和torch.cuda.set_rng_state获取和重置pytorch的随机数的种子(1KB以内的内存数据，不占显存)
输入:
    cache_limit_size: 最大缓存大小限制(字节)
    gpu_devices: 需要在计算前存随机数种子的显卡设备索引/索引列表，可选值：
        "all": 默认值，所有显卡，不清楚用的哪张显卡就用这个
        "none": 不存随机数种子（如果模型里没有如dropout之类需要用随机数的网络层，可以不存随机数种子）
        整数: 显卡索引，从0开始，比如有两张显卡，就是存在显卡索引0和1
        列表: 显卡索引列表，例: [0,1,2]
...
4) 训练示例3：
如果模型做了切分，在显卡和cpu上分别计算，或者在不同显卡上计算，需要为每张需要限制缓存的显卡创建一个recals，因为每个recals只会控制被它调用的模型的缓存总量，不管这些模型的缓存是保存在同一张显卡还是不同显卡，还是有部分在显卡，有部分在内存
