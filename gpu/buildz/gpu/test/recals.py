...
def unit(din, num_ln, num_attrn, num_conv, num_heads=8, k_size = 3, shape_conv = None, shape_out = None, dropout_rate=0):
    ...
    '''
    nets = []
    num_max = max(num_ln, num_attrn, num_conv)
...
def save(fp):
    ...
    log.info(f"save: train: {train_data.mean(), eval_train.mean()}, test: {test_data.mean(), eval_test.mean()}")
    dv.xsave(fp, model=model, trainx=train_data, testx=test_data)

def load(fp):
    if not isfile(fp):
        log.info(f"[WRAN] load fp '{fp}' is not exist and will not be load")
        return
    assert isfile(fp), f"data file not exist: {fp}"
    global model, train_data, test_data
    eval_train = eval(model, train_data)
    eval_test= eval(model, test_data)
    log.info(f"before load: train: {train_data.mean(), eval_train.mean()}, test: {test_data.mean(), eval_test.mean()}")
    obj = dv.xload(fp, model=model, trainx=train_data, testx=test_data)
    model, train_data, test_data = obj.model, obj.trainx, obj.testx
    eval_train = eval(model, train_data)
    eval_test = eval(model, test_data)
    log.info(f"load: train: {train_data.mean(), eval_train.mean()}, test: {test_data.mean(), eval_test.mean()}")
...
segments = int(conf.get("segments", 1))
lr=0.001
...
log.info(f"model: {len(model)}, size: {az.fmt_sz(dv.sizes(model))}, train_size: {az.fmt_sz(dv.sizes(train_data))}")
if order=='save':
...
def fc_check_seq(fc, *a, **b):
    return checkpoint_sequential(fc, segments, a[0], use_reentrant=False)
log.info("conf:", conf)
...
if order == "train":
    ...
    size = dv.sizes(model)
    log.info(f"model size: {az.fmt_sz(size)}")
    exit()
else:
    assert False
if not sptable:
...
if order=="cachex":
    ...
    log.info(f"caches:", cal_obj.cache_size(1))
    exit(0)
if order == "cachexs":
    model = model.cuda()
    train_data = train_data.cuda()
    cache_size, max_unit, cache_count = cal_obj.analyzes(model, train_data)
    log.info(f"total cache size: {az.fmt_sz(cache_size)}, max_unit_size: {az.fmt_sz(max_unit)}, cache_num: {cache_count}")
    exit(0)
if order=='testx' or order=='testxs':
    log.info(f"try train")
    train(model, train_data, 1, lr, show_epoch, cal_obj, spt)
    log.info(f"trainable")
    exit(0)
...
if isinstance(cal_obj, recals.ReCals):
    cal_obj.clean()
