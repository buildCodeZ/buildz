...
2，forward的时候在显存不够的时候丢掉前面的缓存，后面重新计算(ReCal)，测试效果不错，用起来也方便
    如果网络层包括随机数（比如dropout层），为了重新计算的时候dropout层计算的结果一样，建议用recal_with_rngs，其会在计算前缓存随机数种子，重新计算的时候使用之前的随机数种子
    
    ReCal实现后才发现有个torch.utils.checkpoint
    和checkpoint区别：
        checkpoint需要手动设置哪些网络层调用checkpoint，在这些网络层设置checkpoint
        ReCal是设置一个最大缓存大小，自动抛弃缓存和反向梯度的时候重新计算
        逻辑上的区别还没看
