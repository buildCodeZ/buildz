...
class Note:
    ...
    def get(self, fc, base=0):
        ...
        if idx not in self.conf:
            self.conf[idx] = {}
            self.conf[idx]['@function']=fc
            self.conf[idx]['type']='function'
            self.conf[idx]['parameters'] = self.analyze_vars(fc,base)
        conf = self.conf[idx]
    ...
    def _var(self, name, _base, **maps):
        def wrap(fc):
            conf = self.get(fc, _base)
            inputs = conf['parameters']['properties']
