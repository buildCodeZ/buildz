...
class OpenAiApi(Api):
    @staticmethod
    def from_conf(conf):
        return OpenAiApi(conf.get("url"), conf.get("api_key"))
    ...
    def send_dict(self, send:dict)->tuple[dict, dict]:
        ...
        rst = dz.maps(role=msg.role,  content=msg.content, think=msg.reasoning, tool_calls=self.out_tool_calls(msg.tool_calls))
        return rst, usage
