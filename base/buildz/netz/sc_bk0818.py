...
def verify(skt, cert, prv_key, sid=None, cas=None, cls_crypt = cryptz.BlockCrypt, log=None):
    obj = verify_yield(skt, cert, prv_key, sid, cas, cls_crypt, log)
    while True:
        rst = wrap_yield(obj)
        if type(rst) !=str:
            return rst
...
def wrap_yield(obj):
    try:
        ...
        return None
def verify_yield(skt, cert, prv_key, sid=None, cas=None, cls_crypt = cryptz.BlockCrypt, log=None):
    '''
        双向验证证书和生成双向加密密码
        运行逻辑：
        发送本地证书
